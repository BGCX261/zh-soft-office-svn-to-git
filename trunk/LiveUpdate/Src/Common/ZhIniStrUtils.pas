...
* 单元作者：刘啸 (passin@cnpack.org)
* 备    注：从 CnIni 单元分离而来
* 开发平台：PWin2000Pro + Delphi 5.0
* 兼容测试：PWin9X/2000/XP + Delphi 5/6
* 本 地 化：该单元中的字符串均符合本地化处理方式
* 单元标识：$Id: ZhIniStrUtils.pas,v 1.4 2006/09/29 05:33:35 passion Exp $
    ...
    try
      ...
      while Pos <= Length(Str) do begin
        ...
        case I of
          ...
          3: if S <> '' then Font.Style := StringToFontStyles(S);
...
begin
  Result := Def;
  S := Str;
  if (S[1] in csLefts) and (S[Length(S)] in csRights) then begin
...
begin
  Result := Def;
  S := Str;
  if (S[1] in csLefts) and (S[Length(Str)] in csRights) then begin
    Delete(S, 1, 1); SetLength(S, Length(S) - 1);
  end;
  I := Pos(',', S);
  if I > 0 then begin
